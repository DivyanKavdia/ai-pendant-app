...
async function workerTests(){
  ...
  assert.equal(await fetch(scope+'?from=home'),'./index.html?v=1.0.0-prod2');
  ...
  let reply;handlers.message({data:{type:'GET_VERSION'},source:{postMessage:d=>reply=d}});assert.equal(reply.version,'1.0.0-prod2');assert.equal(reply.release,'1.0.0');
}
(async()=>{
