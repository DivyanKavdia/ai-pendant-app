...
async function workerTests(){
  ...
  assert.equal(await fetch(scope+'?from=home'),'./index.html?v=1.0.0-device1');
  assert.equal(await fetch(scope+'app.js?v=1.0.0-device1','cors'),'./app.js?v=1.0.0-device1');
  ...
  let reply;handlers.message({data:{type:'GET_VERSION'},source:{postMessage:d=>reply=d}});assert.equal(reply.version,'1.0.0-device1');assert.equal(reply.release,'1.0.0');
}
(async()=>{
