...
async function workerTests(){
  ...
  assert.equal(await fetch(scope+'?from=home'),'./index.html?v=5.6.1');
  assert.equal(await fetch(scope+'app.js?v=5.6.1','cors'),'./app.js?v=5.6.1');
  assert.equal(await fetch(scope+'ota.js?v=5.6.1','cors'),'./ota.js?v=5.6.1');
  assert.equal(await fetch(scope+'api','cors','POST'),undefined);
  let reply;handlers.message({data:{type:'GET_VERSION'},source:{postMessage:d=>reply=d}});assert.equal(reply.version,'5.6.1');
}
(async()=>{
