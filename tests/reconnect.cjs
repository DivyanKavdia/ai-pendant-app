...
function context(devices = []) {
  ...
  const c = {firmwareBusy:false,console, Boolean, Number, String, Date, Error, Promise,
    ...
    clearTimeout() {}, log(label,message) {if(label === "Reconnect") calls.push(["reconnect-note",message]);}, toast() {}, friendlyError: e => e.message,
...
async function recoveryTests() {
  ...
  assert(t.calls.some(x=>x[0]==='reconnect-note'&&x[1].includes('cannot restore')));
...
async function workerTests(){
  ...
  assert.equal(await fetch(scope+'?from=home'),'./index.html?v=1.0.0-timeline1');
  assert.equal(await fetch(scope+'app.js?v=1.0.0-timeline1','cors'),'./app.js?v=1.0.0-timeline1');
  assert.equal(await fetch(scope+'ota.js?v=1.0.0-timeline1','cors'),'./ota.js?v=1.0.0-timeline1');
  ...
  let reply;handlers.message({data:{type:'GET_VERSION'},source:{postMessage:d=>reply=d}});assert.equal(reply.version,'1.0.0-timeline1');assert.equal(reply.release,'1.0.0');
}
(async()=>{
