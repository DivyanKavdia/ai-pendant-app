...
test('production shell includes reliability UX, pendant health, forced OTA cache migration and browser-only storage copy',()=>{
  ...
  assert.match(sw,/OTA_ASSET = "\.\/ota\.js\?v=1\.0\.0-ota4"/);assert.match(sw,/url\.pathname === otaUrl\.pathname/);
