/* Production release feed for device-ID OTA: multi-target S3/C3 with legacy ES256 + GitHub provenance migration. */
(function(root){
  'use strict';
  const TARGET='esp32s3-fh4r2-qspi-4m';
  const TARGETS=Object.freeze({
    [TARGET]:Object.freeze({target:TARGET,chip:9,flashBytes:4194304,psramBytes:2097152,partition:'default',maxSize:0x140000,
      marker:'SYNAP-ESP32S3-OTA-ID-V3',manifestPath:'latest.json',releasePrefix:''}),
    'esp32c3-supermini-4m':Object.freeze({target:'esp32c3-supermini-4m',chip:5,flashBytes:4194304,psramBytes:0,partition:'default',maxSize:0x140000,
      marker:'SYNAP-ESP32C3-OTA-ID-V3',manifestPath:'targets/esp32c3-supermini-4m/latest.json',releasePrefix:'targets/esp32c3-supermini-4m/'})
  });
  ...
  function validateManifest(m) {
    ...
    if(!config||(!legacy&&!signed&&!github)||m.protocol!==3||m.chip!==config.chip||m.partition!==config.partition||
      m.flashBytes!==config.flashBytes||m.psramBytes!==config.psramBytes||!integer(m.build,504,65535)||
      !integer(m.size,288,config.maxSize)||!/^\d+\.\d+\.\d+$/.test(m.version)||
      !/^[a-f0-9]{64}$/.test(m.sha256)||!/^[a-f0-9]{40}$/.test(m.commit)||
      m.identity!==`SYNAP-FW:${config.target}:${m.version}:${m.build}`||m.url!==expectedUrl(config,m.build,m.sha256))
      throw Error('Invalid or incompatible firmware release manifest.');
  ...
  async function verifyGitHubProvenance(manifest,fetcher=root.fetch.bind(root)){
    const config=configFor(manifest.target);if(!config)throw Error('Unknown firmware hardware target.');
    ...
    const file=await json(`${API}/contents/${config.manifestPath}?ref=${tip.sha}`,65536,fetcher);
    ...
    if(canonicalManifest(bound)!==canonicalManifest(manifest)||JSON.stringify(bound.provenance)!==JSON.stringify(manifest.provenance))
      throw Error('Verified GitHub release commit does not contain this firmware manifest.');
  ...
  function selectCatalogManifest(value,identity){
    const running=targetFromIdentity(identity);
    if(!running)throw Error('Unknown pendant hardware. Reconnect and check the firmware identity.');
    const selected=value.manifests[running.target];
    if(!selected)throw Error(`No production firmware is published for ${running.target}.`);
    return selected;
  }
  function materializeCatalog(value,identity){
    const selected=selectCatalogManifest(value,identity);
    for(const key of Object.keys(value))delete value[key];
    Object.assign(value,selected);
    return value;
  }
  function compatible(m,info,identity){
    if(isCatalog(m))materializeCatalog(m,identity);
  ...
  async function latest(fetcher=root.fetch.bind(root)){
    ...
    let index;
    try{index=await json(BASE+'targets.json?t='+Date.now(),16384,fetcher);}catch(_){return primary;}
    ...
    for(const [target,path] of Object.entries(index.targets)){
      ...
      if(!config||path!==config.manifestPath)continue;
      if(target===primary.target)continue;
      const manifest=await verifyManifest(await json(BASE+path+'?t='+Date.now(),8192,fetcher),fetcher);
      if(manifest.target!==target||manifest.build!==index.build)throw Error('Firmware target catalog contains mismatched releases.');
  ...
  async function download(m,capacity,fetcher=root.fetch.bind(root),signal){
    const manifest=await verifyManifest(m,fetcher),config=configFor(manifest.target);
    if(manifest.size>capacity)throw Error('Firmware does not fit this pendant.');
    ...
    const marker=new TextEncoder().encode(manifest.identity+'\0');let found=false;
    for(let i=0;i<=bytes.length-marker.length;i++){if(marker.every((b,j)=>bytes[i+j]===b)){found=true;break;}}
    if(!found)throw Error('Firmware image target/build does not match the release.');
    if(!config)throw Error('Unknown firmware hardware target.');
    return new Blob([bytes],{type:'application/octet-stream'});
  }
  root.SynapReleases={TARGET,TARGETS,REPOSITORY,RELEASE_BRANCH,WORKFLOW,BASE,API,IDENTITY_UUID,LEGACY_UNSIGNED_MAX_BUILD,SIGNING_KEY_ID,SIGNING_PUBLIC_KEY_SPKI,
    canonicalManifest,validateManifest,verifyManifest,verifyGitHubProvenance,targetFromIdentity,compatible,latest,download,isCatalog,selectCatalogManifest};
