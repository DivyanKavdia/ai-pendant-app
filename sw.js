...
const APP_SHELL = [
  ENTRY_PATH, "./theme.js?v=1.0.0-wireframe1", "./device-identity.js?v=1.0.0-device1", OTA_ASSET, RELEASES_ASSET, "./styles.css?v=1.0.0-diag1", "./brand.css?v=1.0.0-wireframe1", "./audio-store.js?v=1.0.0-prod2", "./enhancements.js?v=1.0.0-prod2", "./app.js?v=1.0.0-diag1", "./ai-providers.js?v=1.0.0-ai2", "./recording-bridge.js?v=1.0.0-touch1",
...
self.addEventListener("activate", event => {
  event.waitUntil(caches.keys().then(keys => Promise.all(keys.filter(key => key.startsWith("dk-pendant-pwa-") && key !== CACHE_NAME).map(key => caches.delete(key)))).then(() => self.clients.claim()));
});
self.addEventListener("message", event => {
  if (event.data && event.data.type === "GET_VERSION" && event.source) event.source.postMessage({ type: "APP_VERSION", version: APP_REVISION, release: APP_VERSION, revision: APP_REVISION });
});
self.addEventListener("fetch", event => {
  ...
  const isEntry = request.mode === "navigate" && url.origin === scope.origin && (url.pathname === scope.pathname || url.pathname === scope.pathname + "index.html");
  if (isOta || isReleases) {
